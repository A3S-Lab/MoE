...
/// Validated tokenizer paired with an OLMoE model vocabulary.
#[derive(Clone)]
pub struct OlmoeTokenizer {
    inner: Tokenizer,
...
mod tests {
    ...
    fn validates_vocab_and_roundtrips_text() {
        ...
        assert!(OlmoeTokenizer::from_file(&path, 2).is_err());
        let tokenizer = OlmoeTokenizer::from_file(&path, 4).unwrap();
