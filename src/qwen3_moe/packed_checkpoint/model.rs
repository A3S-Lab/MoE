...
use crate::{packed_expert_tensor_name, MoeError, PackedExpertRecord, Result};
...
/// Validated packed Qwen3-MoE checkpoint bound to one Power runtime and its
/// hard resource limits.
#[derive(Clone)]
pub struct Qwen3MoePackedCheckpoint {
    root: PathBuf,
    config: Qwen3MoeConfig,
    manifest: Qwen3MoePackedManifest,
    dense_store: Arc<WeightStore>,
    expert_store: Arc<WeightStore>,
    runtime: EmbeddedRuntime,
}

impl Qwen3MoePackedCheckpoint {
    pub fn open(root: impl AsRef<Path>, runtime: EmbeddedRuntime) -> Result<Self> {
        ...
        dense_store.verify_integrity(
            "packed Qwen3-MoE dense weights",
            &manifest.dense_weights_sha256,
        )?;
        expert_store.verify_integrity(
            "packed Qwen3-MoE expert weights",
    ...
    pub fn manifest(&self) -> &Qwen3MoePackedManifest {
        &self.manifest
    }
    ...
    pub fn execution_batch_binding(&self) -> Result<ExecutionBatchBinding> {
        let state_layout = ExecutionDigest::utf8_text(&format!(
            "a3s-moe-qwen3-kv-cache-f32-v1\0{}\0{}\0{}\0{}\0{:?}",
        ...
        let scheduler = ExecutionDigest::utf8_text(
            "a3s-moe-qwen3-mixed-layer-ragged-expert-union-greedy-scheduler-v1",
        );
    ...
    pub fn load_tokenizer(&self) -> Result<Qwen3MoeTokenizer> {
        ...
        Qwen3MoeTokenizer::from_file(self.root.join(TOKENIZER_FILE), self.config.vocab_size)
    }
    ...
    pub fn load_cpu_streaming(&self, policy: ResidencyPolicy) -> Result<Qwen3MoeStreamingModel> {
...
impl std::fmt::Debug for Qwen3MoePackedCheckpoint {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter
            .debug_struct("Qwen3MoePackedCheckpoint")
...
fn read_metadata(root: &Path) -> Result<(PathBuf, Qwen3MoeConfig, Qwen3MoePackedManifest)> {
    ...
    enforce_metadata_limit(&config_path, MAX_CONFIG_BYTES, "packed Qwen3-MoE config")?;
    let config = Qwen3MoeConfig::from_json_path(&config_path)?;
    let manifest_path = root.join(MANIFEST_FILE);
    enforce_metadata_limit(
        &manifest_path,
        MAX_MANIFEST_BYTES,
        "packed Qwen3-MoE manifest",
    )?;
    let manifest: Qwen3MoePackedManifest = serde_json::from_slice(&std::fs::read(&manifest_path)?)?;
...
fn validate_manifest(
    ...
    if manifest.schema != Qwen3MoePackedManifest::SCHEMA {
        return Err(MoeError::InvalidConfig(format!(
    ...
    let expected_expert_files = sparse_layers
...
fn validate_dense_inventory(store: &WeightStore, config: &Qwen3MoeConfig) -> Result<()> {
...
fn validate_expert_inventory(
    ...
    let sparse_layers = (0..config.num_hidden_layers)
        .filter(|layer| config.is_sparse_layer(*layer))
        .collect::<Vec<_>>();
    let expected_count = sparse_layers
        .len()
...
mod tests {
    ...
    fn packed_checkpoint_is_send_and_sync() {
        ...
        assert_send_sync::<Qwen3MoePackedCheckpoint>();
    }
}
