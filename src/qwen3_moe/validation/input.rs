...
use crate::qwen3_moe::Qwen3MoeConfig;
use crate::{MoeError, Result};

use super::{
    Qwen3MoeCheckpoint, Qwen3MoeOracleFile, Qwen3MoePublicOracle, QWEN3_MOE_PUBLIC_MODEL_ID,
...
pub(super) fn read_oracle(path: &Path) -> Result<Qwen3MoePublicOracle> {
...
pub(super) fn validate_provenance(oracle: &Qwen3MoePublicOracle) -> Result<()> {
    for (found, expected, label) in [
        (
            oracle.schema.as_str(),
            QWEN3_MOE_PUBLIC_ORACLE_SCHEMA,
            "oracle schema",
        ),
        (
            oracle.model.id.as_str(),
            QWEN3_MOE_PUBLIC_MODEL_ID,
            "model ID",
        ),
        (
            oracle.model.revision.as_str(),
            QWEN3_MOE_PUBLIC_MODEL_REVISION,
            "model revision",
        ),
        (
            oracle.model.transformers_revision.as_str(),
            QWEN3_MOE_TRANSFORMERS_REVISION,
            "Transformers revision",
        ),
        (
            oracle.model.transformers_dtype.as_str(),
            QWEN3_MOE_TRANSFORMERS_DTYPE,
            "Transformers dtype",
        ),
    ] {
        if found != expected {
    ...
    if oracle.model.transformers_source_sha256 != QWEN3_MOE_TRANSFORMERS_SOURCE_SHA256 {
        return Err(MoeError::InvalidConfig(format!(
            "Transformers source SHA-256 must be '{QWEN3_MOE_TRANSFORMERS_SOURCE_SHA256}', found '{}'",
...
pub(super) fn validate_token_ids(encoded: &[u32], oracle: &Qwen3MoePublicOracle) -> Result<()> {
...
pub(super) fn validate_oracle_shape(
    ...
    for layer in 0..config.num_hidden_layers {
        let sparse = config.is_sparse_layer(layer);
        let router = &oracle.output.layer_router_logits[layer];
        let routes = &oracle.output.layer_routes[layer];
        if router.is_some() != sparse || routes.is_some() != sparse {
            return Err(MoeError::InvalidTensor(format!(
                "oracle layer {layer} does not match the configured sparse schedule"
            )));
        }
        let (Some(router), Some(routes)) = (router, routes) else {
            continue;
        };
