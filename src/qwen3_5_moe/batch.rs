...
use super::{Qwen36MoeCache, Qwen36MoeStreamingModel};

/// One independently cached Qwen3.6 session in a route-unioned step.
pub struct Qwen36MoeStreamingBatchRow<'a> {
    token_ids: &'a Tensor,
    cache: &'a mut Qwen36MoeCache,
}

impl<'a> Qwen36MoeStreamingBatchRow<'a> {
    pub fn new(token_ids: &'a Tensor, cache: &'a mut Qwen36MoeCache) -> Self {
    ...
    pub fn cache(&self) -> &Qwen36MoeCache {
        self.cache
    }
}

impl std::fmt::Debug for Qwen36MoeStreamingBatchRow<'_> {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter
            .debug_struct("Qwen36MoeStreamingBatchRow")
...
#[derive(Debug)]
pub struct Qwen36MoeStreamingBatchRowOutput {
    pub logits: Tensor,
...
#[derive(Debug)]
pub struct Qwen36MoeStreamingBatchOutput {
    pub rows: Vec<Qwen36MoeStreamingBatchRowOutput>,
    pub layer_union_routes: Vec<RoutedExpertBatch>,
    pub layer_staging: Vec<StagedWeightBatchReport>,
}

impl Qwen36MoeStreamingModel {
    /// Executes ragged sessions together, staging one exact expert union per
    /// layer while retaining independent GDN and KV state for every row.
    pub async fn forward_batch(
        &self,
        rows: &mut [Qwen36MoeStreamingBatchRow<'_>],
        permit: &ExecutionPermit,
        cancellation: &CancellationToken,
    ) -> Result<Qwen36MoeStreamingBatchOutput> {
        ...
        let mut row_router_logits = (0..rows.len())
            .map(|_| Vec::with_capacity(self.routed_mlps.len()))
            .collect::<Vec<_>>();
        let mut row_routes = (0..rows.len())
            .map(|_| Vec::with_capacity(self.routed_mlps.len()))
            .collect::<Vec<_>>();
        let mut layer_union_routes = Vec::with_capacity(self.routed_mlps.len());
        ...
        for (layer, routed_mlp) in self.routed_mlps.iter().enumerate() {
            ...
            for row in 0..rows.len() {
                let row_attended = self.dense.forward_mixer(
                    layer,
                    &hidden_states[row],
                    &mut next_caches[row],
                    positions[row],
            ...
            let routed = routed_mlp
                ...
                .await?;
            check_cancellation(cancellation)?;
            let union_hidden = routed
                .hidden_states
                .reshape((total_positions, self.config().hidden_size))?;
            let union_logits = routed
                .router_logits
                .reshape((total_positions, self.config().num_experts))?;
            let mut start = 0_usize;
            for row in 0..rows.len() {
                let row_length = sequence_lengths[row];
                let row_routed = union_hidden.narrow(0, start, row_length)?.reshape((
                    1,
                    row_length,
                    self.config().hidden_size,
                ))?;
                let shared = self.dense.forward_shared_expert(layer, &normalized[row])?;
                hidden_states[row] = attended[row].add(&row_routed.add(&shared)?)?;
            ...
            layer_union_routes.push(routed.routes);
            layer_staging.push(routed.staging);
        }

        let mut outputs = Vec::with_capacity(rows.len());
        for row in 0..rows.len() {
            outputs.push(Qwen36MoeStreamingBatchRowOutput {
        ...
        Ok(Qwen36MoeStreamingBatchOutput {
            rows: outputs,
            layer_union_routes,
            layer_staging,
        })
...
mod tests {
    ...
    fn public_batch_outputs_are_send_and_sync() {
        ...
        assert_send_sync::<Qwen36MoeStreamingBatchRowOutput>();
        assert_send_sync::<Qwen36MoeStreamingBatchOutput>();
    }
}
