...
| M5 GPU execution | In progress | Device-native graph, explicit fallback evidence, and Qwen3.6 CUDA public-checkpoint parity pass; Metal public-checkpoint parity and optimized kernels remain. |
...
| M9 Model-neutral speculative decoding | In progress | Public Power proposal, adaptive-length, and exact-acceptance primitives pass picolm parity; transactional draft/verify and confidence-aware scheduling must pass exactness and throughput gates with trained adapters on Qwen3.6 and at least one other architecture. |

Detailed ownership and acceptance rules live in
[docs/architecture.md](docs/architecture.md).
The M9 ownership split and staged delivery plan are specified in
